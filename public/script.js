(function () {
  ...
  function updateBackground() {
    ...
    document.body.style.background =
      `radial-gradient(circle at 18% 0%, hsl(${h1}, 70%, 72%) 0%, transparent 40%),` +
      `radial-gradient(circle at 90% 0%, hsl(${h2}, 70%, 52%) 0%, transparent 36%),` +
      `radial-gradient(circle at 50% 100%, hsl(${(h1 + 220) % 360}, 65%, 14%) 0%, #050509 72%)`;
    scoreEl.style.color = `hsla(${h1}, 18%, 80%, 0.26)`;
  }
  ...
  function loop() {
    ...
    if (!gameEnded) {
      const targetYBase = stack.length * BOX_HEIGHT + 6;
      const sway = combo > 0 ? Math.sin(t * 0.01) * 0.12 * Math.min(combo, 5) : 0;
      const targetY = targetYBase + sway;
      ...
      camera.position.x = 12 + jx;
      camera.position.z = 12 + jz;
      camera.lookAt(0, 0, 0);
    }
